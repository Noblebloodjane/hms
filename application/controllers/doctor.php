...

{
	...
	function accept_appointment($param1 = '', $param2 = '', $param3 = '') {
		...
		if ($param1 == 'accept') {
			...
			// Delete the corresponding request from 'request_app'
			$this->db->where('appointment_id', $param2);
			$this->db->delete('request_app');
	
			$this->session->set_flashdata('flash_message', get_phrase('appointment_created'));
	
			redirect(base_url() . 'index.php?doctor/accept_appointment', 'refresh');
		}
		...
		if ($param1 == 'decline') {
			...
			// Delete the request from 'request_app' table
			$this->db->where('appointment_id', $param2);
			$this->db->delete('request_app');
		...
		$page_data['page_name'] = 'accept_appointment';
		$page_data['page_title'] = get_phrase('accept_appointment');
		$page_data['request_appointments'] = $this->db->get('request_app')->result_array();
